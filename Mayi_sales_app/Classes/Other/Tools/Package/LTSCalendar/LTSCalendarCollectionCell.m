...
- (void)setSelected:(BOOL)isSelected animated:(BOOL)animated{
    ...
    if(isSelected){
        if(!self.item.isOtherMonth || [LTSCalendarAppearance share].isShowSingleWeek){
            circleView.color = [UIColor clearColor];
            circleView.layer.borderColor = [UIColor colorWithRed:228/255.0 green:27/255.0 blue:61/255.0 alpha:1].CGColor;
        ...
        if ([self isToday]){
            ...
            textLabel.textColor = [UIColor whiteColor];
            circleView.color = [UIColor colorWithRed:228/255.0 green:27/255.0 blue:61/255.0 alpha:1];
            textLabel.text = @"今";
