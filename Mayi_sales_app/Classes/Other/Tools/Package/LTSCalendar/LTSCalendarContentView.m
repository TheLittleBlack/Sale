...
- (void)setEventSource:(id<LTSCalendarEventSource>)eventSource{
    ...
    [self updatePageWithNewDate:NO isScrollEvent:NO];
...
//            }
    ...
    //     我改的
    [self updatePageWithNewDate:NO isScrollEvent:YES];
...
//{
//
//   [self updatePageWithNewDate:!isOwnChangePage isScrollEvent:NO];
//    [UIView performWithoutAnimation:^{
//        [self.collectionView reloadData];
//    }];
//
...
//    }];
    if (self.eventSource && [self.eventSource respondsToSelector:@selector(calendarDidLoadPageCurrentDate:)]) {
        [self.eventSource calendarDidLoadPageCurrentDate:[self getNewCurrentDate]];
    }
...
- (void)getDateDatasisScroll:(BOOL)isScroll{
...
- (void)updatePageWithNewDate:(BOOL)isNew isScrollEvent:(BOOL)isScrollEvent
{
//
    //如果是显示周 默认 选中该周第一天
